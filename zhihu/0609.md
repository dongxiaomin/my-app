...
function Child (props) {
  return sdiv>子组件接收到数据：{props.name}</div>
}
```
...
1. Action: Action 只是一个简单的json对象，type 和有payload作为键。type 是必须要有的，payload是可选的。
...
2. Action Creators：这些是创建Actions的函数，因此我们在派发action时不必在组件中手动编写每个 action。
...
3. Reducers：Reducers 是纯函数，它将 action和当前 state 作为参数,计算必要的逻辑并返回一个新的state。 这些 Reducers 没有任何副作用。 它不会改变 state 而是总是返回 state 。
...
connect和bindActionCreators来自 redux.

connect: 用于连接 store
bindActionCreators: 将 action creators 绑定到的 props中
